...
|----|-----------|-----------|
| BUG-2608-01 | **Reproducido — defecto real** | `PatientSheet.tsx:331` — el botón "Editar ficha" tiene `disabled` hardcodeado. También "Nueva licencia" (`:339`) y "Agendar control" (`:347`). Al pulsarlos no hay diálogo ni request. El backend **sí** expone `PUT /api/v1/ingresos/...`: falta solo la interfaz. |
